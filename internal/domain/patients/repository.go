...
func (repo *patientRepository) Store(patient *Patient) (*Patient, error) {
	_, err := repo.db.NamedExec("INSERT INTO patients (id, name, age, gender, address, phone, email, observations, created_at, updated_at) VALUES (:id, :name, :age, :gender, :address, :phone, :email, :observations, :created_at, :updated_at)", patient)
